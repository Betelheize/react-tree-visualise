...
const returnDetector_1 = require("./returnDetector");
...
function activate(context) {
    ...
    const returnDetector = new returnDetector_1.ReturnDetector();
    ...
    function analyzeComponentHierarchy(content, functionName) {
        const components = [];
        let depth = 0;
        let maxDepth = 0;
        ...
        let complexity = 'low';
        if (components.length > 3 || maxDepth > 3)
            complexity = 'high';
        else if (components.length > 1 || maxDepth > 1)
            complexity = 'medium';
        // ELEGANT HIERARCHY DETECTION
        let hierarchy = 'child';
        // Root components: Main page/container components
        if (functionName && (functionName.includes('Page') ||
    ...
    function createASCIIComponentTree(content) {
    ...
    function createJSXStructureTree(jsxContent) {
    ...
    function createSimpleElementTree(elementName, isComponent) {
        const icon = isComponent ? '⚛' : '▢';
        return `└─ ${icon} ${elementName}\n  ├─ opening tag\n  ├─ content area\n  └─ closing tag`;
    }
    // ENHANCED JSX STRUCTURE: Show the actual structure like your example
    function createEnhancedJSXTree(elementName, isComponent, depth = 0) {
        const icon = isComponent ? '⚛' : '▢';
        const indent = '  '.repeat(depth);
        return `${indent}└─ ${icon} ${elementName}\n${indent}  ├─ <${elementName}>\n${indent}  ├─ content\n${indent}  └─ </${elementName}>`;
    }
    // NESTED COMPONENT TREE: Show nested structure with depth
    function createNestedComponentTree(elementName, isComponent, depth) {
        const icon = isComponent ? '⚛' : '▢';
        const indent = '  '.repeat(depth);
    ...
    function findJSXElementsInDocument(document, returnInfo) {
        const jsxElements = [];
    ...
    function analyzeJSXContent(content) {
        const trimmed = content.trim();
        if (trimmed === 'null' || trimmed === 'undefined') {
    ...
    function updateDecorations(editor) {
        ...
        const fragmentReturns = [];
        const nullReturns = [];
        const componentTreeReturns = [];
        const clickableReturns = [];
        // ELEGANT HIERARCHICAL DECORATIONS
        const rootComponentReturns = [];
        const parentComponentReturns = [];
        const childComponentReturns = [];
        const nestedChildReturns = [];
        // VISUAL RETURN PREVIEW DECORATIONS
        const visualReturnPreviews = [];
        const componentHeavyReturns = [];
        const elementHeavyReturns = [];
        const simpleReturnPreviews = [];
        // INLINE JSX TREE DECORATIONS
        const inlineJSXTrees = [];
        const inlineComponentTrees = [];
